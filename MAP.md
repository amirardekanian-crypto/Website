...
| [`habits.html`](habits.html) | **Proof** — daily habit coach & tracker. Onboarding → Today · Awards · Coach · You. 8 default habits + custom, XP/levels, achievements, per-habit history, reactive coach card, real coach chat. Same `?client=<id>&key=<key>` link as the program; syncs via the existing `save_progress`/`get_progress` RPCs (keys `<id>_hab_cfg` / `<id>_hab_log`). Demo: `?client=demo` | — |
