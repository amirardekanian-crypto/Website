...
| [`jump.html`](jump.html) | **Jump test** — measures countermovement jump and squat jump height, drop-jump RSI and the 10-5 repeated hop test from a phone video. Entirely on-device: the video is read in the browser, nothing is uploaded, there is no account and no backend. Opens as coaching (what the test is for, how to film it, how to perform it) and only then asks for a clip. Refuses clips under 60 fps, and blocks contact-time metrics under 120 fps, because below that the measurement error is bigger than the change it would be reporting. **All the knowledge lives outside the page**, one file per thing that might change: [`assets/js/jump/`](assets/js/jump/) holds the physics, the camera guide, the athlete questions and one file per test. Companion dev page [`jump-selftest.html`](jump-selftest.html) runs synthetic jumps with a known answer against the engine. `noindex`, not in the nav | [`assets/js/jump/README.md`](assets/js/jump/README.md) |
