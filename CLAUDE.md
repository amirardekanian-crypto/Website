...
- **The course app's pictures (Amir, 2026-09-19).** AI-made (GPT Image 2 and Higgsfield), one look: shadows lean deep green,
  ...
  **`scripts/grade_tps_art.py`**, which applies ONE shared colour grade (prompts drift off-colour, the grade does not) and
  exports 1080 px, 16:9, about 40 KB each. They show in the **demo** (`/tennis/app/?demo=1`), where all 13 of its pictures exist: `ART_KINDS`, `ART` and
  `ART_V` in `app.js` decide where each goes (block covers by block number, lessons and tests by id, one tarp picture for
  every locked page, one room per session card, one for the session-complete screen). ⚠️ **Buyers see the Programme tab's pictures, not the lessons' or tests' (2026-09-20)**: `ART_KINDS` lists the kinds shown to everyone (block, place, testday, done); lessons, tests and the locked pages stay demo-only until all 22 lessons and 7 tests have a cover (14 and 3 do), or a buyer's lesson list is half pictures. `tennis/app/sw.js` keeps each picture for offline use, in its own `tps-art` cache, the first time it is shown. Rules every picture follows: no text or logos in
