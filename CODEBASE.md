...
- **Weekly quests.** Three a week, Monday→Sunday, worth real XP, shown on Today under the nudge. Four kinds, all measured from the log alone (`daysHit:<habit>`, `total:<habit>`, `qualify`, `perfect`), paid **on the day they complete** so they ride the same dated-bonus rail as tiers and milestones. Which three are live is **derived, never stored** — an explicit prescription in `questWeeks` or a deterministic pick seeded on the week's Monday, run identically by `questsForWeek()` and `public.hab_week_quests` (same string hash). **Amir prescribes a week** with `select public.set_quests('2026-07-27', array['w_water5', …]);`. Pool lives on the `xp_rules` row **and** as `QUEST_POOL` in habits.html (offline fallback) — keep them in step. SQL: [`supabase/stage13_weekly_quests.sql`](supabase/stage13_weekly_quests.sql), which also holds the current `hab_bonus_xp`. See [`XP_SYSTEM.md`](XP_SYSTEM.md) §8.5.
