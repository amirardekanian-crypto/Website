...
- **Everything is computed from real logs.** Streaks, per-habit best/week/35-day rate, XP, level and every achievement are derived from the athlete's own history — nothing is seeded. XP is 30 per habit completion + 10 per perfect day; levels use the progressive `LEVEL_AT` thresholds (a flat curve handed out LEGEND in a month).
...
- **Edit this when:** Changing the default habit list (the `HABITS` array), the achievement set (`ACHIEVEMENTS`, each with a `measure`), coach copy (`coachCopy()`), or the level curve (`LEVEL_AT`).
