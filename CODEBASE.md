...
- A **Supabase** database (added after this site first launched) that backs up athlete progress to the cloud and powers the coach dashboard (it also carried two-way messaging until 2026-09-26, when that moved to WhatsApp)
...
- **Every picture in the app comes from `assets/art/`** (2026-09-19), including the 13 Library shelf pictures (7 Sessions + 6 Playbook) — whose paths live in `public.library_categories.banner`, with the index JSONs as the offline fallback, and which also fill each workout card's thumbnail and the header of each workout's own screen (`libBannerFor()`). Ten cycle-card
  ...
  mirrored with a clay scrim, which is why no image may carry text. New local keys, all
  plain values that ride the normal snapshot: `<id>_art_finish` (which finish picture is
...
- **There is no chat.** The app never messages the coach — it runs in a fixed "nudge and recap" mode: one reactive nudge card on Today whose button opens whichever habit is being avoided, plus a rolling seven-day recap. Athlete↔coach messaging is WhatsApp: the button is at the top of `program.html`'s Coach tab, and neither app has carried a chat since 2026-09-26.
...
- **Depends on:** Supabase (`supabase-js` from a CDN, the `call_logs` / `cycle_reports` tables, plus `programs` / `athlete_progress` / `session_history` as inputs), Google sign-in. Self-contained — no shared partials.
...
The site started with no backend. It now uses **Supabase** (a hosted Postgres database) so athlete progress survives a phone wipe, and the coach dashboard has something to read. (It carried coach↔athlete messaging too, until that moved to WhatsApp on 2026-09-26.) The pages talk to Supabase directly over the internet — there is no server of yours to run or maintain.
...
- **What they are:** The exact SQL that built the live database, kept in the repo as a record and so it can be rebuilt. They're applied in order and each one is safe to re-run.
  ...
  - `stage2_keys.sql` — per-athlete secret keys, so an athlete's link (`program.html?client=<id>&key=<key>`) could write only their own data. ⚠️ **Superseded 2026-09-07:** `athlete_keys` is empty, those links are refused, and identity comes from a signed-in session (`athlete_identities` / `current_athlete_id()`).
  - `stage3_messages.sql` — the two-way messaging table. ⚠️ Nothing reads or writes it since 2026-09-26, when the in-app chat was removed from both apps; the table and its functions are kept so no past thread is lost.
  ...
  - `assess_01_accounts_content_data.sql` — **the tennis testing app** (`/tennis-testing/app/`, a Farsi app for coaches and academies, sold from the product page `/tennis-testing/`; its source and content live in the private `assess-content` repo, and only the shell is copied here). Buyer logins in `assess_accounts` (made by the `assess-login` Edge Function from coach.html → **Testing app**, on `assess.<username>@amirardekani.com`; revoke bans and keeps the data), the paid content in `assess_content`, and each account's `assess_players` / `assess_results` / `assess_notes`. Public sign-up is still on in this project, so every policy also requires an active `assess_accounts` row; buyers never get an `athlete_identities` row, and the coach has no policy to read buyer data. Content is published from the private repo through `assess_publish()`, guarded by a secret whose sha256 is in `assess_publish_keys`.
  - `tps_01_accounts_content.sql` — **the Tennis Performance System course app** (`/tennis/app/`, the paid Farsi handbook for players; its source and content live in the private `tps-content` repo, and only the shell is copied here; the demo was built directly in `tennis/app/` on 2026-09-15, so copy those files into that repo before its next deploy, and run `python scripts/stamp_tps_app.py` after any edit here). Buyer logins in `tps_accounts` (made by the `tps-login` Edge Function from coach.html → **Course**, on `tps.<username>@amirardekani.com`; revoke bans, and the app then deletes its offline copy) and the handbook in `tps_content`: one row per content file plus a `manifest` row, each versioned by a hash of its body so phones download only what changed. Nothing about the buyer is stored. Public sign-up is still on in this project, so reading the content also requires an active `tps_accounts` row; buyers never get an `athlete_identities` row. Content is published from the private repo through `tps_publish()`, guarded by a secret whose sha256 is in `tps_publish_keys`.
  - `stage30_session_log.sql` — **The Card Remembers** (2026-09-24). `session_history.log` holds each session's sets as data (`[{ex, block, reps, sets:[{w,n,r,d}], note}]`, circuits `{ex, circuit, items, rounds}`) beside the text `summary`; `save_session()` stores it and keeps the old value when a stale app copy sends none. `get_my_history(p_athlete_id)` lets an athlete read **only their own** last 180 days (coach reads anyone; `demo` gets `[]`; a NULL id is refused). Rows from before this stage have no `log` and come back with their `summary`, which `program.html`'s `parseSummaryLog()` reads, so no data was migrated. The app keeps an offline copy in `<id>_histcache`, which `_snapshot()` never syncs. UI: `paintLastTime()` / `openHistorySheet()` in `program.html`.
  - `stage31_spine.sql` — **The Spine** (2026-09-24): `exercises` (purpose, pattern, cues, `easier`/`harder`/`alts` as ids, loads, video, `status` draft/approved) and `exercise_coach` (SFR rank, restriction flags; since stage39 also the muscle `credits` and `cost` tier the checker counts volume from), both coach-only by RLS. `get_exercises()` (anyone) returns approved entries only and never the coach table. Edited in coach.html → **Exercises** (`renderExercises()`); read by program.html (`refreshSpine()`, cached as `spinecache`, never synced) for the ⓘ About sheet (Regressions · Progressions · Alternatives, where a link is an id or a plain name: stage34), and cues on cards that have none of their own.
  - `stage38_publish_cycle.sql` — **one-call publish** (2026-09-26): `publish_cycle()` archives the finished cycle from the live row, advances it, writes the new workouts/notes and splices the coaching log, all in one transaction, and returns `programme_fingerprint()` for the check. Coach and service only (`execute` revoked from anon and authenticated). Called by `/program-assemble` Step 7.
  - `stage39_spine_credits.sql` — **the volume count on the Spine** (2026-09-26): `exercise_coach.credits` (muscle → 1 or 0.5, VOL-10) and `.cost` (heavy/moderate/isolation/none, VOL-2), checked by `spine_credits_ok()`. `scripts/check_program.py` counts every programme's volume tables and day loads from them; coach.html → Exercises → an entry → *Coach only* edits them. The muscle list exists four times (that function, `SPINE_MUSCLES` in coach.html, `MUSCLES` in check_program.py and in the spine skill's `draft_sql.py`).
  - `stage32_qualities.sql` — **The Quality Map** (2026-09-24): `qualities` (the ten, draft/approved, coach-only by RLS) and `get_qualities()` (anyone, approved only); `exercise_coach.suggested_qualities` (Claude's tags for already-approved entries). Tags live in `exercises.qualities` (first = primary). program.html: `qualityMix()` → chips on the Home day cards, `openQualitySheet()`; cached as `qualcache`, never synced. coach.html → Exercises: the ten texts, the tag editor, and the per-athlete *Quality check*.
  - `tps_02_demo.sql` — **the course demo** (`/tennis/app/?demo=1`). `tps_demo()`, callable by anyone, builds the demo from the live rows on every call: week 1 and every exercise it uses, the broad jump test with the test-day rules, three lessons, and the start page and video library whole. Every other lesson, test and exercise goes out as its card only (name, icon, one line, `locked: true`). That is what makes the lock real: `tps_content` stays buyer-only, and nothing locked reaches the phone. The picks are the `cfg` arrays at the top of the function; a pick that stops existing is listed by `select public.tps_demo()->'demo'->'missing';`.
