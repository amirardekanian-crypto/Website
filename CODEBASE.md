...
#### `habits.html` — Proof, the habit coach & tracker (private)
- **What it does:** A standalone gamified habit tracker with a coaching layer, built to the PROOF design handoff. A 3-step onboarding (goals → which habits → coach volume) then four tabs: **Today** (hero + reactive coach card + habit list), **Awards** (level, XP, 9 achievements), **Coach** (real chat with Amir), **You** (settings). Eight default habits — STRENGTH, STEPS, SLEEP, WATER, FUEL, BREATHE, MOBILITY, SUPPS (vitamin D · creatine · omega-3) — plus custom habits the athlete adds. Check-off habits toggle; counter habits (steps, sleep, water, fuel) take a number via `+` or the log sheet. Athletes open it with the **same link pattern as the program**: `habits.html?client=<id>&key=<key>`. Demo mode: `?client=demo` (local-only, no cloud).
- **Everything is computed from real logs.** Streaks, per-habit best/week/35-day rate, XP, every level and every achievement are derived from the athlete's own history — nothing is seeded or stored.
- **Progression (League-of-Legends style).** *Every habit has its own level* plus one overall level, so an athlete can be GRINDER 2 on water and ROOKIE 5 on strength at the same time. Ranks are 10 names × 5 sub-levels (`RANKS`): ROOKIE · CONTENDER · GRINDER · OPERATOR · ENFORCER · PREDATOR · MACHINE · RELENTLESS · UNTOUCHABLE · IMMORTAL. Level 6 is CONTENDER 1, level 20 is OPERATOR 5. Past level 50 the athlete **prestiges** — the ladder restarts carrying a star (`★1 ROOKIE 1`), so there is no cap.
- **The XP curve.** `levelCost(base, n) = base × n^0.55`, so every level costs more than the last (level 19→20 is 4.4× level 1→2). Tuned to two anchors Amir set: **one full day of habits reaches level 2** (a perfect day is 420 XP, level 2 costs 300) and **~2 months of solid (80%) adherence reaches level 20** (19,270 XP ≈ 57 days). Habit ladders reuse the curve with `base` scaled to that habit's own daily value, so all habits level at the same *rate* — which is what makes your level in a habit track how often you actually do it.
- **XP is weighted by health impact:** STRENGTH 100 · STEPS 60 (target 10,000) · SLEEP 50 · FUEL 40 · WATER 30 · MOBILITY 30 · BREATHE 20 · SUPPS 20; custom habits get `CUSTOM_XP`. Counter habits pay pro-rata as you go and `COMPLETION_BONUS` (1.2×) once finished.
- **Level-up celebration.** A full-screen game-style takeover (masked ray burst, scale-punched badge, shockwave, flying shards, animating XP bar) fires for an overall level or a genuine **rank promotion**; routine habit levels get an inline `.lvchip` flash in their row instead, so day one isn't nine consecutive takeovers. Two variants: near-black with clay rays for a level, full clay for a rank-up. Queued and dismissed one at a time.
- **Level-up detection** never stores levels. `CFG.seen` holds the last level the athlete was *shown* and `checkLevelUps()` diffs against it — which covers local check-offs, a cloud sync that arrives carrying new XP, and the first-run migration alike. First run seeds the baseline silently so an athlete with history isn't ambushed by twenty queued celebrations.
- **Consistency ladders.** Every habit has a 5-tier consecutive-day ladder (`CONSISTENCY_TIERS`): KINDLED 5d · STEADY 10d · LOCKED IN 20d · IRONCLAD 30d · UNBREAKABLE 60d. Tiers are earned on the *best run ever*, so breaking a streak never revokes a badge. These sit alongside the nine one-off `ACHIEVEMENTS`.
- **Design:** PROOF's modernist structure (zero border-radius, 2px section rules, 1px row rules, flush-left labels, typographic — no icons) on the **AA Performance palette**: green `#0E4A36` is the accent, clay `#C7552F` carries the coach card and mono meta, paper `#FAF7F2` ground. Barlow Condensed 900 display / Barlow body / Space Mono counters. Optional dark mode in Settings.
- **If deleted:** Athletes lose the habit tracker. The program app and dashboard are unaffected.
- **Depends on:** `data/<id>.json` (athlete name + key fallback) and **Supabase** — but only *existing* functions: `save_progress` / `get_progress` for habit data (localStorage keys `<id>_hab_cfg` and `<id>_hab_log`, which merge into the same `athlete_progress` row the program app uses), and `get_messages` / `send_athlete_message` / `mark_athlete_read` for the Coach tab. **No new tables or functions.**
- **Two deliberate departures from the handoff:** the palette is AA's, not the handoff's red modernist one (the bundle's own Color Options sheet includes this mapping); and STEPS/SLEEP are entered manually rather than synced from HealthKit, which a web app can't read.
- **Edit this when:** Changing the default habit list or its XP weights (the `HABITS` array), the rank names (`RANKS`), the pacing (`XP_BASE` / `XP_GROWTH` — see the two anchors above before touching these), the consistency tiers (`CONSISTENCY_TIERS`), the one-off achievements (`ACHIEVEMENTS`, each with a `measure`), or coach copy (`coachCopy()`).
- **Don't touch:** The Supabase URL/key and the key-resolution block. The `migrateV1()` fold-forward (it rescues data from the first shipped version). `noindex` on purpose.
