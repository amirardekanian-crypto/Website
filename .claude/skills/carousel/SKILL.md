...
0. **`Content/BUILD-MAP.md`** — the master system index (skim first; it links everything). Has the "where do I find X?" table, the brand marks (icon/OG/logo/coach photo/**the orange tennis ball**/court backgrounds) with exact paths (§2), the colour canon (§3), both websites EN+FA (§4), the app screen-by-screen (§5), the full image library — cycle/day/athlete/session banners + the court bgs (§8), and canonical strings/pricing (§10). **The current carousel look = a real tennis-court background (`court-sessions.jpg` green / `court-playbook.jpg` clay) with the orange ball (`tennis-ball-clay.png`) on a dashed clay rally-arc + tennis-ball pagination — see `Carousel-Kit.html`.**
