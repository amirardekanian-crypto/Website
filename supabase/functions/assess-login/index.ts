// Creates, resets, revokes and restores logins for the tennis testing app ("assess").
//
// Same pattern as athlete-login (read its header before changing this). Buyers have no email
// here, so an account is keyed on assess.<username>@amirardekani.com, an address that never
// receives mail. Amir picks the username in coach.html; the app adds the rest when they sign in.
...
//
// Unlike athlete-login, create never touches an existing username (a typo must not reset
// another buyer's password), and revoke BANS the user instead of deleting it, so the buyer's
// players and results are kept and restore can undo it.
...
Deno.serve(async (req: Request) => {
  ...
  let body: { action?: string; username?: string; label?: string };
  ...
  const { data: acct } = await admin.from('assess_accounts')
  ...
  if (action === 'create') {
    ...
    const { data: made, error: mkErr } = await admin.auth.admin.createUser({
      ...
      user_metadata: { assess_username: username },
    });
    if (mkErr || !made?.user) return json({ error: mkErr?.message || 'could not create user' }, 400);
    const { error: linkErr } = await admin.from('assess_accounts').insert({
      user_id: made.user.id, username, label, initial_password: password, password_set_at: now,
    });
    if (linkErr) {
      // Never leave a login that maps to no account: it would sign in to an empty app.
  ...
  if (action === 'reset' || action === 'restore') {
    ...
    await admin.from('assess_accounts').update({
  ...
  if (action === 'revoke') {
    ...
    if (error) return json({ error: error.message }, 400);
    await admin.from('assess_accounts').update({ revoked_at: now, initial_password: null }).eq('user_id', acct.user_id);
