...
| **EN mantra (2026-07-06)** | **"A path to your target." → "A coach in your pocket."** — the English setup→payoff pair, same shape as the Farsi mantra ("coach" in clay = «مربی» in clay). A CTA needing just one line uses "A coach in your pocket." alone. **One-line canonical form (Amir, 2026-07-08): "A coach in your pocket, a path to your target."** — use this order when the mantra appears as a single statement (it closes the EN homepage's Platform section as a pull-quote; second half in clay). Setup→payoff order still fine for reel/CTA outros. |
...
| I need… | Grab | Notes |
|---|---|---|
| **App art — the current set** (2026-09-19) | [`../assets/art/cycles/`](../assets/art/cycles/) — 10 WebP, 1200×675 · [`../assets/art/moments/`](../assets/art/moments/) — 7 WebP | The whole athlete app, one look: **no people, no logos, no text, no yellow**; green-black shadows, cream highlights, clay the only saturated colour. Cycle art is keyed by FAMILY (bedrock·iron·build·armour·voltage·spring·brakes·engine·reset·peak), not by cycle name. Moments = session complete ×3, new best ×3, welcome. Made in Higgsfield (GPT Image 2.5 Flare) from the shot list, graded through one pass, gated on yellow %, text-zone luminance and file size. Specs + prompts: [`../IMAGES.md`](../IMAGES.md) §0. ⚠️ Replacements ship as `-v2`, never over the same path (`sw.js` is cache-first). |
...
- Chrome: no top nav; bottom tab bar (Home / Coach / Library / Game Plan), green icons, active gets glow.
