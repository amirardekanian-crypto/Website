...
| Frame border **.30** opacity dark / .18 light (`margin:28px; radius:14px`) | `.16` opacity (still in kit CSS + DESIGN_SYSTEM §3 — stale) |
...
| **Wordmark / share banner** | [`../assets/img/og-image.png`](../assets/img/og-image.png) (1200×630) | Master: [`og-source.png`](../assets/img/source/og-source.png) (1731×909). ⚠ live file is 548KB vs the <100KB budget in IMAGES.md. |
...
- ⚠ Reels 1–3 predate the rules (gold accents, old outro wording, reel-1's non-canonical CTA) — copy the *mechanics*, not the styling. Reel-4's handle `@amirardekani` is unconfirmed (see flags).
...
1. **Instagram handle:** DESIGN_SYSTEM §0 + reel-4 use `@amirardekani`; §10 + site footer use `@amirardekanian`. Confirm the canonical one and sweep.
2. **og-image.png is 548KB** (budget <100KB) — recompress; it ships on every link share.
3. **29 cycle slugs have no banner art** (green fallback shows); existing 9 banners are bright-studio style vs the documented moody grade.
4. **Reels 1–3 + kit outro wording** still carry gold accents / old mantra («کامل», comma) — pending a sweep.
5. **instagram-cards** are pre-brand neon-yellow — next results card should be redesigned on the current system.
6. DESIGN_SYSTEM §3/§4 still describe the `.16` frame + retired counter/bar — the kit + this atlas are the ruling.
