...
| `w_qualify5` | **FIVE ON TARGET** | 5 days on target | 220 |
...
| HALF DECENT | 3 days on target | `qualify` · 3 | 120 |
| SEVEN FROM SEVEN | Every day on target | `qualify` · 7 | 420 |
...
| `qualify` | N days that clear the gate (`dayQualifies()` — currently 80% of what you could do, or at most one thing undone) |
...
| "No zero days" | Would need a per-quest `qualify` threshold, not the one global gate |
